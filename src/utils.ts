...
export function getEventData(event: NostrEvent): EventData {
  ...
  if (event.kind === 30023) {
    ...
    extractedSummary = shortenUrlsAndIdentifiers(event.content).slice(0, 500) + '...';
  }
...
export function shortenUrlsAndIdentifiers(extractedSummary: string): string {
  // Regex to match URLs and nostr identifiers
  const urlAndNostrRegex = /(https?:\/\/\S+|nostr:\w+)/gi;

  return extractedSummary.replace(urlAndNostrRegex, (match) => {
    // Remove "nostr:" prefix if present
    const cleanMatch = match.startsWith('nostr:') ? match.slice(6) : match;
    return cleanMatch.length > 5 ? `${cleanMatch.slice(0, 12)}...` : cleanMatch;
  });
}
